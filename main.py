...
async def command_start_handler(message: Message):
    ...
    if connected:
        ...
        await message.answer(
            text="Выберите кошелёк для подключения:", reply_markup=mk_b.as_markup()
        )
...
async def connect_wallet(message: Message, wallet_name: str):
    ...
    await message.answer(
        text="Подключите кошелёк в течение 3 минут", reply_markup=mk_b.as_markup()
    )
    ...
    for i in range(1, 180):
        ...
        if connector.connected:
            if connector.account.address:
                ...
                await message.answer(
                    f"Вы подключены с адресом <code>{wallet_address}</code>",
